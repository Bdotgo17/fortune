...
app.post('/api/create-checkout', async (req, res) => {
  ...
  try {
    ...
    purchases.set(id, { paid: false, price: FORTUNE_PRICE_CENTS, shopifyResponse: r.data });
...
app.get('/simulate-checkout/:id', (req, res) => {
  ...
  return res.send(`<html><body><h1>Simulated Checkout</h1><p>Pay $${(FORTUNE_PRICE_CENTS/100).toFixed(2)} to see your fortune.</p><form method="POST" action="/simulate-checkout/${id}/pay"><button type="submit">Pay $${(FORTUNE_PRICE_CENTS/100).toFixed(2)}</button></form></body></html>`);
});
...
app.listen(PORT, () => console.log(`Fortune backend running on http://localhost:${PORT}`));
